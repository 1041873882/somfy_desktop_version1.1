...
int mFT::start(void)
{
	...
	FT_New_Face(m_library, "/dnake/fonts/Roboto-Regular.ttf", 0, &m_normal);
	FT_New_Face(m_library, "/dnake/fonts/Roboto-Light.ttf", 0, &m_light);
	FT_New_Face(m_library, "/dnake/fonts/Roboto-Condensed.ttf", 0, &m_condensed);

	this->setPixel(24);
	return 0;
}

int mFT::load(FT_ULong uc, int family)
{
	if (family == 1)
		return this->loadLight(uc);
	else if (family == 2)
		return this->loadCondensed(uc);
	return this->loadNormal(uc);
}

int mFT::loadNormal(FT_ULong uc)
{
	int err = -1;
	FT_UInt index = FT_Get_Char_Index(m_normal, uc);
	if (index > 0) {
		err = FT_Load_Glyph(m_normal, index, FT_LOAD_DEFAULT | FT_LOAD_NO_BITMAP);
		if (!err) {
			err = FT_Render_Glyph(m_normal->glyph, FT_RENDER_MODE_NORMAL);
			m_glyph = m_normal->glyph;
			m_bmp = m_normal->glyph->bitmap;
		}
	}
	return err;
}

int mFT::loadLight(FT_ULong uc)
{
	int err = -1;
	FT_UInt index = FT_Get_Char_Index(m_light, uc);
	if (index > 0) {
		err = FT_Load_Glyph(m_light, index, FT_LOAD_DEFAULT | FT_LOAD_NO_BITMAP);
		if (!err) {
			err = FT_Render_Glyph(m_light->glyph, FT_RENDER_MODE_NORMAL);
			m_glyph = m_light->glyph;
			m_bmp = m_light->glyph->bitmap;
		}
	}
	return err;
}

int mFT::loadCondensed(FT_ULong uc)
{
	int err = -1;
	FT_UInt index = FT_Get_Char_Index(m_condensed, uc);
	if (index > 0) {
		err = FT_Load_Glyph(m_condensed, index, FT_LOAD_DEFAULT | FT_LOAD_NO_BITMAP);
		if (!err) {
			err = FT_Render_Glyph(m_condensed->glyph, FT_RENDER_MODE_NORMAL);
			m_glyph = m_condensed->glyph;
			m_bmp = m_condensed->glyph->bitmap;
		}
	}
	return err;
}
...
{
	FT_Set_Pixel_Sizes(m_normal, 0, val);
	FT_Set_Pixel_Sizes(m_light, 0, val);
	FT_Set_Pixel_Sizes(m_condensed, 0, val);
}
