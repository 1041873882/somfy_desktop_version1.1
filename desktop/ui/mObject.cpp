...
int mObject::sys_refresh = 0;
mObject *mObject::sys_focus = NULL;
...
void mObject::load(dxml &p, const char *zone)
{
	...
	sprintf(s, "/style/%s/w", zone);
	m_width = p.getInt(s, 0);
	sprintf(s, "/style/%s/h", zone);
	m_height = p.getInt(s, 0);
...
{
	if (sys_focus == this && sys_focus->m_focus == val)
		return;

	if (val) {
		if (sys_focus != NULL)
			sys_focus->setFocus(0);
		sys_focus = this;
	} else {
		if (sys_focus == this)
			sys_focus = NULL;
