			...
			if (y >= 0) {
				for(int j=0; j<ft.height(); j++) {
					for(int i=0; i<ft.width(); i++) {
						...
						fb_pixel(x2+i+m_x+ft.left(), y2+j+m_y, c);
					}
				}
			}
			x2 += ft.advance().x/64;
...
void mText::setFocus(int val)
{
	if (val) {
		setColor(m_focus_color);
	} else {
...
void mText::setStyle(const char *s)
{
	...
	const char *family = strstr(s, "family:");
	const char *focus_color = strstr(s, "focus_color:");
	...
	if (color) {
		...
		sscanf(color, "%x", &val);
		this->setColor(val);
		this->m_normal_color = val;
	}
	if (focus_color) {
		focus_color += strlen("focus_color:");
		int val;
		sscanf(focus_color, "%x", &val);
		this->m_focus_color = val;
	}
	if (left) {
		left += strlen("left:");
